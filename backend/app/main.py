...
from .routers.duplicate import create_router
...
# 为duplicate路由提供全局服务实例
duplicate_router = create_router(duplicate_service)
...
if getattr(sys, 'frozen', False) and hasattr(sys, '_MEIPASS'):
    # 打包环境：静态文件在PyInstaller临时目录的backend/static中
    base_dir = Path(sys._MEIPASS)
    static_dir_path = str(base_dir / "backend" / "static")
else:
    # 开发环境：静态文件在当前目录的static中
    static_dir_path = "static"

if os.path.exists(static_dir_path):
    # 挂载静态资源文件夹到根路径，以匹配前端构建的绝对路径
    ...
    async def read_index():
        """根路径，返回前端首页"""
        return FileResponse(os.path.join(static_dir_path, "index.html"))
    ...
    async def serve_react_app(full_path: str):
        ...
        # 检查是否是静态文件
        static_file_path = os.path.join(static_dir_path, full_path)
